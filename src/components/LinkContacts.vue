...
<script>
    import {findContactsName } from '../services/Associated'
    ...
    export default {
        ...
        methods: {
            // 搜索
            onSearch(value) {
                findContactsName(this.templateId, value).then((res) => {
                    if (res.code == 200) {
            ...
            //查询全部客户
            getFindAllCustomer() {
                findContactsName(this.templateId).then((res) => {
                    if (res.code == 200) {
                        this.customerList = res.rows
                    }
                })
